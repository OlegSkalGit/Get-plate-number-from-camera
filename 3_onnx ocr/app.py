...
INFERENCE_INTERVAL = 0.8     # Детекція раз на 0.8 с
SCREENSHOT_INTERVAL = 10.0   # Знімок повного кадру кожні 10 с
PLATE_SAVE_INTERVAL = 4.0    # Пауза між збереженнями номера в базу
...
class ANPRViewerApp:
    def __init__(self, root):
        ...
        # Очищення старих файлів через 5 секунд після старту, далі раз на 30 хвилин
    ...
    def _check_and_delete_old_files(self):
        """Видаляє номери і скріншоти, старші за delete_older годин, та очищає порожні папки дат."""
        try:
    ...
    def _build_ui(self):
        ...
        self.side_panel = tk.Frame(self.main_body, bg="#1e1e24", width=340)
        self.side_panel.pack_propagate(False)

        side_header = tk.Frame(self.side_panel, bg="#282832", padx=10, pady=8)
        side_header.pack(fill=tk.X, side=tk.TOP)
        tk.Label(side_header, text="Історія розпізнаних номерів", fg="#00e5ff", bg="#282832", font=("Segoe UI", 10, "bold")).pack(side=tk.LEFT)
        ...
        self.plates_tree.column("#0", width=310, stretch=True)
    ...
    def refresh_saved_list(self):
        """Парсинг файлів plate_[ЧАС]_[НОМЕР]_[ТОЧНІСТЬ]%.jpg."""
            ...
            for fname in files:
                if fname.lower().endswith((".jpg", ".png", ".jpeg")):
                    ...
                    if len(parts) >= 4:
                        ...
                        plate_num = parts[2]
                        conf_str = parts[3]
                        disp_str = f"🚘 {plate_num} [{conf_str}]  ({time_disp})"
                    elif len(parts) >= 3:
                        time_disp = ":".join(parts[0].split("-")[:3])
                        plate_num = "_".join(parts[2:])
                        disp_str = f"🚘 {plate_num}  ({time_disp})"
                    elif len(parts) == 2:
                        time_disp = ":".join(parts[0].split("-")[:3])
                        plate_num = parts[1]
                        disp_str = f"🚘 {plate_num}  ({time_disp})"
                    else:
                        disp_str = f"🚘 {clean_name}"

                    if day_key not in grouped_data:
                        grouped_data[day_key] = []
                    grouped_data[day_key].append((disp_str, fpath))
    ...
    def _insert_single_plate_to_tree(self, day_str, disp_str, fpath):
        ...
        item_id = self.plates_tree.insert(day_node, 0, text=f"  {disp_str}", tags=("plate_tag",))
        self.tree_item_map[item_id] = fpath
    ...
    def _show_plate_in_preview(self, fpath):
        ...
        try:
            ...
            self.preview_info_lbl.configure(text=f"Файл: {base}")
    ...
    def _show_help(self):
        help_text = (
            ...
            "• Збереження номерів:\n"
            "  /plates/РРРР-ММ-ДД/plate_[ЧАС]_[НОМЕР]_[ТОЧНІСТЬ]%.jpg\n"
            "• Збереження скріншотів:\n"
            "  /screenshots/РРРР-ММ-ДД/screenshot_[ЧАС].jpg\n"
            "• Автоочищення: старше delete_older годин\n\n"
    ...
    def _ai_worker(self):
        """ШІ-потік: детекція, вирівнювання та оптичний аналіз тексту."""
        ...
        while self.is_running:
            ...
            try:
                ...
                now = time.time()
                # 3. Збереження: plate_[ЧАС]_[НОМЕР]_[ТОЧНІСТЬ]%.jpg
                if new_plate_img is not None and (now - last_plate_save_time >= PLATE_SAVE_INTERVAL):
                    ...
                    plate_filename = f"plate_{time_filename}_{recognized_text}_{conf_pct}%.jpg"
                    ...
                    if self.sidebar_visible:
                        ...
                        disp_str = f"🚘 {recognized_text} [{conf_pct}%]  ({time_display})"
                        self.root.after(0, lambda d=day_folder, s=disp_str, p=plate_path: self._insert_single_plate_to_tree(d, s, p))
    ...
    def _capture_worker(self, source):
        ...
        while self.is_running and cap.isOpened():
            ...
            # 1. Збереження скріншотів по днях і часу
            if self.save_screenshots and (now - last_screenshot_time >= SCREENSHOT_INTERVAL):
                ...
                day_shot_dir = os.path.join(self.screenshots_dir, day_folder)
                os.makedirs(day_shot_dir, exist_ok=True)

                shot_path = os.path.join(day_shot_dir, f"screenshot_{time_filename}.jpg")
            ...
            # Збільшений оверлей номера у правому верхньому кутку (тільки картинка в рамці)
